...
if args.energy:
    # Use VOS logistic scores if available, otherwise fall back to energy
    if id_data['logistic_score'] is not None and ood_data['logistic_score'] is not None:
        print("Using VOS logistic regression scores")
        id_score = -torch.stack(id_data['logistic_score']).cpu().data.numpy()
        ood_score = -torch.stack(ood_data['logistic_score']).cpu().data.numpy()
    else:
        print("Logistic scores not available, using plain energy (this will give poor results)")
