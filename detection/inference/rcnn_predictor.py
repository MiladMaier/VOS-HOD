...
class GeneralizedRcnnPlainPredictor(ProbabilisticPredictor):
    ...
    def generalized_rcnn_probabilistic_inference(self,
        ...
        # Compute VOS logistic scores during inference
        logistic_score = None
        if hasattr(self.model.roi_heads, 'logistic_regression') and hasattr(self.model.roi_heads, 'weight_energy'):
            # Compute weighted energy score using trained weight_energy
            with torch.no_grad():
                # Use the VOS log_sum_exp with learned weights
                energy_scores = self.model.roi_heads.log_sum_exp(inter_feat[:, :-1], 1)
                # Compute logistic regression scores
                logistic_output = self.model.roi_heads.logistic_regression(energy_scores.view(-1, 1))
                # Get probability of being in-distribution (class 1)
